...
export function NavdataPanel({
  ...
  const filtered = useMemo(() => {
    ...
    return f
      ? waypoints.filter((w: any) => w.name.includes(f)).slice(0, 200)
      : waypoints.slice(0, 200);
  }, [waypoints, filter]);
  const filteredGates = useMemo(() => {
    const f = gateFilter.trim().toUpperCase();
    return f
      ? gates
          .filter((g: any) => g.icao.includes(f) || g.label.toUpperCase().includes(f))
          .slice(0, 200)
      : gates.slice(0, 200);
  }, [gates, gateFilter]);
  ...
  async function handleFileUpload(e: any, kind: string) {
    ...
    try {
      ...
      if (kind === "sct") {
        ...
        onParseSct(r);
        setStatus(
          `Loaded ${file.name}: ${r.waypoints.length} wpts, ${r.airports.length} apt, ${r.runways.length} rwy`,
        );
      } else {
        const r = parseESE(text);
        onParseEse(r);
        setStatus(
          `Loaded ${file.name}: ${r.positions.length} positions · ${r.stars.length} STARs · ${r.copx.length} COPX · ${r.gates.length} gates`,
        );
      }
    } catch (err: any) {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder="2511"
            className="w-24 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200 font-mono"
          />
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={!waypoints.length && !positions.length}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 disabled:bg-slate-800 disabled:text-slate-600 rounded text-xs text-slate-200"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {waypoints.length > 0 ? (
                <div className="flex gap-2 items-center">
                  ...
                  <button onClick={onResetSct} className="text-xs text-rose-400 underline">
                    Clear
                  </button>
                </div>
              ) : (
            ...
            {navMeta?.sctAt && (
              <p className="text-xs text-slate-500 mb-1">Parsed {fmtTs(navMeta.sctAt)}</p>
            )}
            <div className="flex gap-2 mb-2">
              <label className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 rounded text-xs text-white cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) => handleFileUpload(e, "sct")}
                  className="hidden"
                />
              </label>
            ...
            <textarea
              value={sctText}
              onChange={(e) => setSctText(e.target.value)}
              placeholder="[FIXES]&#10;MOPIL N050.13.53.000 E004.07.49.000"
              className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs font-mono text-slate-300 focus:border-sky-500 focus:outline-none h-36"
            />
            <button
              onClick={() => {
                if (!sctText.trim()) {
                  setStatus("Paste .sct first");
                  return;
                }
                const r = parseSectorFile(sctText);
                onParseSct(r);
                setStatus(
                  `${r.waypoints.length} wpts, ${r.airports.length} apt, ${r.runways.length} rwy`,
                );
                setSctText("");
              }}
              className="mt-2 flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded text-sm"
            >
              <Icon name="upload" />
              Parse pasted .sct
            </button>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {positions.length > 0 ? (
                <div className="flex gap-2 items-center">
                  <span className="text-xs text-emerald-400">
                    <Icon name="check" size={12} className="inline" /> {positions.length} pos ·{" "}
                    {stars.length} STARs · {copx.length} COPX · {gates.length} gates
                  </span>
                  <button onClick={onResetEse} className="text-xs text-rose-400 underline">
                    Clear
                  </button>
                </div>
              ) : (
                <span className="text-xs text-slate-500">
                  [POSITIONS] [SIDSSTARS] [COPX] [FREETEXT]
                </span>
              )}
            </div>
            {navMeta?.eseAt && (
              <p className="text-xs text-slate-500 mb-1">Parsed {fmtTs(navMeta.eseAt)}</p>
            )}
            <div className="flex gap-2 mb-2">
              <label className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 rounded text-xs text-white cursor-pointer">
                <Icon name="upload" size={12} />
                ...
                <input
                  ...
                  onChange={(e) => handleFileUpload(e, "ese")}
                  className="hidden"
                />
              </label>
              <span className="text-xs text-slate-500 self-center">or paste below</span>
            ...
            <textarea
              value={eseText}
              onChange={(e) => setEseText(e.target.value)}
              placeholder="[POSITIONS]&#10;LFPG_TWR:..."
              className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs font-mono text-slate-300 focus:border-sky-500 focus:outline-none h-28"
            />
            <button
              onClick={() => {
                if (!eseText.trim()) {
                  setStatus("Paste .ese first");
                  return;
                }
                const r = parseESE(eseText);
                onParseEse(r);
                setStatus(
                  `${r.positions.length} positions · ${r.stars.length} STARs · ${r.copx.length} COPX · ${r.gates.length} gates`,
                );
                setEseText("");
              }}
              className="mt-2 flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded text-sm"
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-slate-500">
                {waypoints.length} · {airports.length} apt · {runways.length} rwy
              </span>
            </div>
            <div className="relative mb-2">
              ...
              <input
                ...
                placeholder="Filter..."
                className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-300"
              />
            </div>
            ...
            {gates.length > 0 && (
              <div className="relative mb-2">
                ...
                <input
                  ...
                  placeholder="Filter ICAO or label..."
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-300"
                />
              </div>
            )}
                  ...
                  {!filteredGates.length && (
                    <tr>
                      <td colSpan={4} className="p-3 text-center text-slate-500">
                        {gates.length ? "No matches" : "No gates parsed"}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {rampAirports.length > 0 || rampConfig ? (
                <div className="flex gap-2 items-center">
                  <span className="text-xs text-emerald-400">
                    <Icon name="check" size={12} className="inline" /> {rampAirports.length} airport
                    {rampAirports.length !== 1 ? "s" : ""}
                    {rampConfig
                      ? ` · config (${Object.keys(rampConfig.aircraftWingspans || {}).length} types)`
                      : ""}
                  </span>
                  <button onClick={onResetRampAgent} className="text-xs text-rose-400 underline">
                    Clear
                  </button>
                </div>
              ) : (
            ...
            <p className="text-xs text-slate-500 mb-2">
              Upload one or more files. <code className="text-slate-400">config.json</code> provides
              aircraft wingspans; per-airport JSONs provide stand catalogues.
            </p>
            {!rampConfig && rampAirports.length > 0 && (
              <div className="text-xs text-amber-400 bg-amber-950/30 border border-amber-900/30 rounded p-2 mb-2 flex items-center gap-1.5">
                ...
                Airports loaded but no <code className="text-amber-300">config.json</code> —
                wingspan check uses WTC fallback values.
              </div>
            )}
            <div className="flex gap-2 mb-2">
              <label className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 rounded text-xs text-white cursor-pointer">
                <Icon name="upload" size={12} />
                Upload RampAgent JSON (multi)
                <input
                  type="file"
                  accept=".json,application/json"
                  multiple
                  onChange={handleRampUpload}
                  className="hidden"
                />
              </label>
            </div>
